...
function MainServer(port){
	...
	this.handle_login = function(msg){
		/*
	...
	this.send_message = function(sender, receiver, msg, send_event, feedback_events){
		...
		if(User.is_login(sender) && User.is_login(receiver)){
			...
			if(!User.is_login(receiver)){
				User.add_one_read_message(receiver, send_event, msg);
			}
		}
	};
	...
	this.events_handler = function(socket){
		...
		socket.on(e.LOGIN, this. handle_login);
		socket.on(e.CHAT_MESSAGE, this. handle_chat);
		socket.on(e.INVITATION, this. handle_invitation);
		socket.on(e.INVITATION_ACCEPT, this. handle_accpet_invitation);
		socket.on(e.LOGOUT, this. handle_logout);
		socket.on(e.DISCONNECT, this. handle_disconnect);
	};
