...
function MessageService(){
    var DataHandler = require('../dao/message_handler.js'),
	...
	this.save_a_new_message = function(msg){
		...
		if(event == 'group'){
			db_handler.save_group_message(sender, message,unique_code,receiver);
		}else{
	...
	this.get_group_offline_messages = function(groupname, lastcode, callback){
		db_handler.get_group_offlines(groupname, lastcode, function(results){
			var messages = parse_group_message(results);
			callback(messages);
		});
	};
