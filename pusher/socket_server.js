...
Chat.prototype.login = function(msg, socket){
	_msg_handler(msg, socket, this._broadcast_login);
};
...
Chat.prototype.send = function(msg, socket){
	_msg_handler(msg, socket, this._send_message);
};
...
Chat.prototype._msg_handler = function(msg, socket, func){
	if(__check_format(msg)){
		__update_clients_socket(msg.from, socket);
		func(msg, socket);
	}else{
...
Chat.prototype.__update_clients_socket = function(username, socket){
	if( !this.clients[username] || !this.clients[username].connected ){
...
Chat.prototype._send_message = function(msg, socket){
	if(msg.to == 'all'){
		...
		if((msg.to in this.clients) && (this.clients[msg.to].connected)){
			this.clients[msg.to].emit('chat message', msg);
		}else{
